...
{
  financial.find({code:req.params.code},protection,function(err,financials)
  {
    res.json(financials);
  });
});
...
router.post('/find/filtering',(req,res)=>
{
  ...
  for(var i=0;i<req.body.length;i++)
  {
    ...
    if(req.body[i].check=='UP') updownFilter[i]={$gte:req.body[i].value};
    else if(req.body[i].check=="DOWN") updownFilter[i]={$lte:req.body[i].value};
  }
